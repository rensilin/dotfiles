...
install_macos_packages() {
	...
	run "$BREW" install git neovim tmux ripgrep fd curl unzip
...
install_linux_packages() {
	if command -v pacman >/dev/null 2>&1; then
		...
		run_as_root pacman -Syu --needed --noconfirm \
			base-devel ca-certificates curl fd git gzip neovim ripgrep tar tmux unzip
	elif command -v apt-get >/dev/null 2>&1; then
		log "Installing Debian/Ubuntu packages"
		run_as_root apt-get update
		run_as_root env DEBIAN_FRONTEND=noninteractive apt-get install -y \
			build-essential ca-certificates curl fd-find git gzip neovim ripgrep tar tmux unzip
	elif command -v dnf >/dev/null 2>&1; then
		log "Installing Fedora/RHEL packages"
		run_as_root dnf install -y \
			ca-certificates curl fd-find gcc gcc-c++ git gzip make neovim ripgrep tar tmux unzip
	elif command -v zypper >/dev/null 2>&1; then
		log "Installing openSUSE packages"
		run_as_root zypper --non-interactive install \
			ca-certificates curl fd gcc gcc-c++ git gzip make neovim ripgrep tar tmux unzip
	elif command -v apk >/dev/null 2>&1; then
		log "Installing Alpine Linux packages"
		run_as_root apk add --no-cache \
			bash build-base ca-certificates curl fd git gzip neovim ripgrep tar tmux unzip
	elif command -v xbps-install >/dev/null 2>&1; then
		log "Installing Void Linux packages"
		run_as_root xbps-install -Sy \
			base-devel ca-certificates curl fd git gzip neovim ripgrep tar tmux unzip
	else
		die "unsupported package manager; install git, curl, neovim, tmux, ripgrep, fd, unzip, and C build tools manually"
	fi
}
...
verify_environment() {
	...
	MISSING=""
	for COMMAND in git nvim rg tmux; do
